...
Check(!display.ExpireRetained(TimeSpan.FromSeconds(30)), "Current translation has no expiry while its source is unchanged");
display.BeginUpdate(TimeSpan.FromSeconds(31));
...
Check(!display.ExpireRetained(TimeSpan.FromMilliseconds(31599)) && display.Text.Length > 0, "Previous translation remains visible during the short grace period");
Check(display.ExpireRetained(TimeSpan.FromMilliseconds(31600)) && display.Text.Length == 0 && !display.IsPrevious, "Slow replacement clears the previous translation at 600 ms");
Check(!display.ExpireRetained(TimeSpan.FromSeconds(32)), "Expired translation clears only once");
display.Show("Next complete translation");
Check(display.Text == "Next complete translation" && !display.IsPrevious, "Late replacement appears normally after old text expires");
display.BeginUpdate(TimeSpan.FromSeconds(33));
display.Show("Fast replacement");
Check(!display.ExpireRetained(TimeSpan.FromSeconds(34)) && display.Text == "Fast replacement" && !display.IsPrevious, "Fast replacement cancels old expiry and stays visible");
display.BeginUpdate(TimeSpan.FromSeconds(35));
display.BeginUpdate(TimeSpan.FromMilliseconds(35200));
display.BeginUpdate(TimeSpan.FromMilliseconds(35599));
Check(display.ExpireRetained(TimeSpan.FromMilliseconds(35600)) && display.Text.Length == 0, "Further text changes, blank transitions, retries, and errors cannot prolong retention");
display.BeginUpdate(TimeSpan.FromSeconds(36));
Check(!display.IsPrevious && !display.ExpireRetained(TimeSpan.FromSeconds(37)), "Updates without a previous translation do not start an expiry");
display.Show("Another translation");
display.BeginUpdate(TimeSpan.FromSeconds(38));
display.Reset();
Check(display.Text.Length == 0 && !display.IsPrevious, "Explicit stop or context change clears retained text immediately");
display.Show("New context translation");
Check(!display.ExpireRetained(TimeSpan.FromSeconds(39)) && display.Text.Length > 0, "Reset cancels the old context's retention timer");
